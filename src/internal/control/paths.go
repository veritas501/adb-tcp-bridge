...
// Order: explicit path, ATB_SOCKET, $XDG_RUNTIME_DIR/atb/atb.sock, $HOME/.atb/atb.sock.
func SocketPath(explicit string) (string, error) {
	if explicit != "" {
		return explicit, nil
	}
	if env := os.Getenv("ATB_SOCKET"); env != "" {
		return env, nil
	}
	if runtimeDir := os.Getenv("XDG_RUNTIME_DIR"); runtimeDir != "" {
		return filepath.Join(runtimeDir, "atb", "atb.sock"), nil
	}
	home, err := os.UserHomeDir()
	if err != nil {
		return "", fmt.Errorf("resolve home for default socket path: %w", err)
	}
	return filepath.Join(home, ".atb", "atb.sock"), nil
}

// LogPath resolves the daemon log file path.
// Order: explicitLog, ATB_LOG, same directory as SocketPath(explicitSocket)/atb.log.
func LogPath(explicitLog string, explicitSocket string) (string, error) {
	if explicitLog != "" {
		return explicitLog, nil
	}
	if env := os.Getenv("ATB_LOG"); env != "" {
		return env, nil
	}
	...
	return filepath.Join(filepath.Dir(socketPath), "atb.log"), nil
}
